...
@app.route('/')
def home():
    return jsonify({"status": "online", "message": "Medisore Backend API is running!"})
...
def pose_endpoint():
    ...
    pose_type = request.form.get('pose_type', 'auto')
    ...
    try:
        ...
        # 고화질 사진을 안전한 크기(1024px)로 축소하여 메모리 절약
        img_array = resize_if_large(img_array, max_dim=1024)

        pose_img, result_data = analyze_posture(img_array, pose_type=pose_type, W0=w0, L0=l0, H0=h0)
        gc.collect()  # 메모리 즉시 청소
        
        if pose_img is None:
            return jsonify({'error': result_data.get('error', '자세 분석 실패')}), 200
...
def download_pdf():
    ...
    logo_path = get_image_path('logo_3.png') or get_image_path('logo.png')
    ...
    def draw_page_template(c, page_num):
        if logo_path: c.drawImage(logo_path, 40, height - 55, width=90, height=40, mask='auto')
        c.setFont('Malgun', 18)
